...
class DanceModel extends fm.ChangeNotifier {
  ...
  var _showNumbers = NUMBERS_COUPLES;
  ...
  void moveSelectedDancer(double dx, double dy) {
    if (_selectedDancer != null) {
      final dancersToMove = <Dancer>[_selectedDancer!];
      if (asCouples) {
        final partner = dancers.firstWhere(
            (d) =>
                d != _selectedDancer &&
                d.numberCouple == _selectedDancer!.numberCouple,
            orElse: () => _selectedDancer!);
        if (partner != _selectedDancer) {
          dancersToMove.add(partner);
        }
      }

      for (final d in dancersToMove) {
        final currentPos = d.starttx.location;
        final newPos = Vector(currentPos.x + dx, currentPos.y + dy);
        d.setStartPosition(newPos);
      }
      notifyListeners();
    }
  }
  ...
  void rotateSelectedDancer(double angle) {
    if (_selectedDancer != null) {
      final dancersToRotate = <Dancer>[_selectedDancer!];
      if (asCouples) {
        final partner = dancers.firstWhere(
            (d) =>
                d != _selectedDancer &&
            ...
            orElse: () => _selectedDancer!);
        if (partner != _selectedDancer) {
          dancersToRotate.add(partner);
        }
      }

      for (final d in dancersToRotate) {
        d.rotateStartAngle(angle);
      }
      notifyListeners();
    }
  }
